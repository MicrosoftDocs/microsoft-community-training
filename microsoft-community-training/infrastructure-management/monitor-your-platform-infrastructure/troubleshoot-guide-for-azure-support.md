...
This document walks through root cause and resolution steps for some of the major issues faced by the customers. Please try the resolution steps before logging an ICM ticket to the CT team.

## Brief on MCT Architecture

The Community Training (CT) platform is available as a [Managed Application](https://learn.microsoft.com/azure/azure-resource-manager/managed-applications/) on the [Azure Marketplace](https://azuremarketplace.microsoft.com/marketplace/apps/project-sangam.microsoft-community-training) for customers. The CT instance is deployed on the customer’s Azure subscription and utilizes the following Azure services:
...
If the error is related to a failure in accessing certain resources, try restarting that resource. If this does not work reach out to    CT support team.

**Root Cause 2**: App service unresponsive because of high resource usage.

**Resolution**: Navigate to the App service plan resource on your Azure portal and check the CPU utilization. If CPU is the bottleneck (greater than 85-90 %) scale up the instance (Note: please don't scale out as there are certain limitations with scale out on CT side).
...
**Root Cause 2**: "/Error" page is visible after the user selects the Sign-in button and completes the username-password flow.
...
* idp:AzureADExternalAuthClientSecret (Azure AD), 
* idp:AzureADB2CExternalAuthClientSecret (Azure AD B2C),
* idp:PhoneExternalAuthClientSecret (Phone).

If the client secret has expired or is nearing expiration, here are [**steps to generate a new client secret and link it to key-vault**](../../frequently-asked-questions/generate-new-clientsecret-link-to-key-vault.md#steps-to-generate-a-new-client-secret-and-link-it-to-key-vault).
...
**Root Cause 1**: The token issued has expired. This error can happen if a user has the course player page open for more than 4 hours.
...
**Root Cause**: Errors of content not loading are generally related to the CORS setting.

**Resolution**: If the customer is using a custom domain or even for the AzureFD or AzureWebsites URL, check if CORS is correctly configured in customer's storage account. Make sure URL has permission for get, post and PUT. Navigate to Storage account of customer's instance and check for 'Resource Sharing (CORS)' and follow below steps:
...
* Save this by selecting the “save” button above.

### Issue 6: Unable to add lessons to courses

**Root Cause 1**: Lessons are not added immediately to the learning path

**Resolution**: Since lessons are added/deleted in an async manner, there can be a delay of few seconds between actually adding lessons to the learning path. Check later and the lessons will be added.

**Root Cause 2**: Delay on the upload process to create new course content, stuck on the "processing" screen.

**Resolution**: If the customer is using a custom domain or even for the AzureFD or AzureWebsites URL, check if CORS is correctly configured in customer's storage account. Make sure URL have permission for get, post and PUT.
...
**Resolution**: Ensure that the lesson is not a media lesson, or the completion criteria is met.
...
**Root Cause**: Profile Field addition requires the doer of the action should be of the authorized role type. Also, the profile field name added cannot be empty, cannot contain certain special characters and cannot be the same as an existing profile field name.
...
**Root Cause 1**: A certificate template PDF file uploaded is corrupted. Most PDF Viewer Software has the ability to fix any corruptions in the PDF when rendering for viewing however, the CT service doesn't support this behavior and, when trying to generate a certificate using a corrupted PDF, the operation will fail.
...
**Resolution**: Locate the required template in the Certificate Templates section (only accessible to Global Administrators), select replace then the original template PDF file from the file system which was being previously used.
